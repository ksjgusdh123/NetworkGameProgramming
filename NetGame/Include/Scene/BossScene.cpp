...
void CBossScene::GameStateCheck(float elapsedTime)
{
	...
	if (!m_inGameData.monster[0].is_alive) {
		m_Etimer += elapsedTime;
		if (m_Etimer > 2.f)
			CSceneManager::GetInst()->CreateScene<CResultScene>();
	}

	if ((m_inGameData.players[0].hp <= 0 && m_inGameData.players[1].hp <= 0))
	{
		CSceneManager::GetInst()->CreateScene<CResultScene>();
	}
}

void CBossScene::RenderPlayerName(HDC hDC)
{
	Vector2 pos;
	...
	for (int i = 0; i < 2; ++i)
	{
		...
		Vector2 Namepos = Vector2(m_inGameData.players[i].pos.x, m_inGameData.players[i].pos.y - 60);
